...
class SingularItemViewModel @Inject constructor(private val userRepository: UserRepository,private val singularRepository: SingularRepository): ViewModel() {
    ...
    fun singularUserInfoActionHandler(action: SingularUserInfoAction) {
        when (action){
            is SingularUserInfoAction.GetUserInfo -> getUserInfoById(action.userId)
    ...
    private fun getUserInfoById(userId: Long) {
        viewModelScope.launch {
            userRepository.getUserInfoById(userId).collect{res ->
                userInfoState = SingularUserInfoState(userInfo = res.data)
            }
        }
    }
}
...
sealed class SingularUserInfoAction {
    data class GetUserInfo(val userId: Long) : SingularUserInfoAction()
